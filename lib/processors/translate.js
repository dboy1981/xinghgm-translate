...
const { normalizeKeyString, normalizeTranslationOutput } = require('../utils/textNormalize')
...
function loadTranslated(filepath, key_name, val_name, sheetname) {
  const translated = {}
  const data = sheetname ? xlsx.readXlsx(filepath, sheetname) : xlsx.readXlsx(filepath)
  ...
  for(var item of data) {
    ...
    if(Array.isArray(key_name)) {
      for(var key_item of key_name) {
        ...
        key += normalizeKeyString(item[key_item])
      }
    } else {
      if(!item[key_name]) continue;
      key = normalizeKeyString(item[key_name])
    }
    if(!key) continue;
    if(key && item[val_name]) {
      translated[key] = normalizeTranslationOutput(item[val_name])
...
exports.trans = function (options) {
  ...
  if(Array.isArray(opt.add_data)) {
    for(var item of opt.add_data) {
      opt.data[item.key] = normalizeTranslationOutput(item.val)
    }
  }
...
exports.create = function (options) {
  ...
  opt.key_filter = function(key, sheet, z) {
    ...
    const cleanKey = normalizeKeyString(key);
